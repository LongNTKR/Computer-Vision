...
class HandDectector():
    def __init__(self, mode=False, maxHands=2, modelComplexity=1, dectectionCon=0.5, trackCon=0.5,
                 path='D:\Code\Python\OpenCV\HandTracking\parameters.h5'):
        self.mode = mode
    ...
    def _get_data(self, img, bias=25):
        for handLms in self.lmList:
            x_min = self._relu(np.min(handLms[:,1])-bias)
            x_max = self._relu(np.max(handLms[:,1])+bias)
            y_min = self._relu(np.min(handLms[:,2])-bias)
            ...
            masks = cv2.rectangle(np.zeros(img.shape[:2], dtype='uint8'), start, end, 255, -1)
            hand = cv2.bitwise_and(img, img, mask=masks)[y_min:y_max, x_min:x_max]
            cv2.rectangle(img, start, end, (255, 0, 0), 2)
        return hand
...
def get_data():
    ...
    while True:
        ...
        dectector.find_hands_land_mark(img, draw=False)
        
        lmList = dectector.find_position(img, draw=False)
        
        if len(lmList) != 0:
            hand = dectector._get_data(img)
            
            cv2.imwrite('D:/Code/Python/OpenCV/HandTracking/Data/5/' + str(count) + '.jpg', hand)
            count += 1
...
def main(fps=False):
    ...
    dectector = HandDectector(path='D:\Code\Python\OpenCV\HandTracking\parameters.h5')
    ...
    while True:
        ...
        if len(lmList) != 0:
            dectector.sign_digits_reconition(img)
